...
func main() {
	...
	if scanner.Scan() {
		sourceFolderPath = strings.TrimSpace(scanner.Text())
		// when dragging in the folder to the terminal
		// in some cases there will be quotation marks
		if string(sourceFolderPath[0]) == "\"" {
			sourceFolderPath = sourceFolderPath[1 : len(sourceFolderPath)-1]
		}
	}
	...
	if scanner.Scan() {
		targetFolderPath = strings.TrimSpace(scanner.Text())
		// when dragging in the folder to the terminal
		// in some cases there will be quotation marks
		if string(sourceFolderPath[0]) == "\"" {
			targetFolderPath = sourceFolderPath[1 : len(sourceFolderPath)-1]
		}
	}
